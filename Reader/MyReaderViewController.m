...
@interface MyReaderViewController ()<UICollectionViewDelegate, PCFontAdjustViewDelegate>

@property (strong, nonatomic) MyModelViewController *modelController;
@property (strong, nonatomic) MyCollectViewModel *collectionViewModel;
@property (strong, nonatomic) MyGlobalModel *globalModel;
...
- (void)viewDidLoad {
    ...
    MyDataViewController *startingViewController = [self.modelController viewControllerAtIndex:0];
...
- (void)setupToolbar
{
    ...
    [self.toolbar setBackgroundImage:[UIImage imageNamed:@"导航"] forToolbarPosition:UIBarPositionAny barMetrics:UIBarMetricsDefault];
    self.toolbar.tintColor = [UIColor whiteColor];
    [self.toolbar setItems:@[item1,fixibleItem,item2,fixibleItem,item3,fixibleItem,item5]];
...
- (void)loadText:(NSString *)text
{
    [self.globalModel loadText:text completion:^{
        ...
        [self pageControllerSetIndex:[MyGlobalModel shareModel].currentPage];
    }];
}

- (void)pageControllerSetIndex:(NSInteger)index
{
    [self.pageController setViewControllers:@[[self.modelController viewControllerAtIndex:index]] direction:UIPageViewControllerNavigationDirectionForward animated:YES completion:nil];
}
...
- (MyModelViewController *)modelController {
    // Return the model controller object, creating it if necessary.
    // In more complex implementations, the model controller may be passed to the view controller.
    if (!_modelController) {
        _modelController = [[MyModelViewController alloc] init];
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    [MyGlobalModel shareModel].currentPage = page;
    [MyGlobalModel shareModel].currentRange = NSRangeFromString([MyGlobalModel shareModel].rangeArray[page]);
...
{
    [[MyGlobalModel shareModel] updateFontCompletion:^{
        self.collectionViewModel.text = self.globalModel.text;
        self.collectionViewModel.attributes = self.globalModel.attributes;
        self.collectionViewModel.dataArray = self.globalModel.rangeArray;
        self.collectionView.delegate = nil;
        [self.collectionView reloadData];
        [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:[MyGlobalModel shareModel].currentPage inSection:0] atScrollPosition:UICollectionViewScrollPositionNone animated:NO];
        [[NSNotificationCenter defaultCenter] postNotificationName:kUpdatePageNotification object:nil];
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[MyPageCell class] forCellWithReuseIdentifier:PageCellIdentifier];
    }
...
- (MyCollectViewModel *)collectionViewModel
{
    if (!_collectionViewModel) {
        _collectionViewModel = [[MyCollectViewModel alloc] init];
    }
    return _collectionViewModel;
}

- (MyGlobalModel *)globalModel
{
    if (!_globalModel) {
        _globalModel = [MyGlobalModel shareModel];
    }
    return _globalModel;
}
...
- (void)plusAction
{
    if ([MyGlobalModel shareModel].fontSize >= 30) {
//        self.plusButton.enabled = NO;
    } else {
        self.plusButton.enabled = YES;
        [MyGlobalModel shareModel].fontSize += 2;
        //更新字体
...
- (void)minusAction
{
    if ([MyGlobalModel shareModel].fontSize <= 14) {
