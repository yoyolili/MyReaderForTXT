...
#import "MyReaderTool.h"

@interface MyDataViewController ()

@property (nonatomic, strong) MyReaderTool *readerTool;
...
- (void)updatePage
{
    [self.progressLabel setText:[NSString stringWithFormat:@"%ld/%ld", (long)[MyGlobalModel shareModel].currentPage + 1, (long)self.totalPage]];
}
...
- (MyReaderTool *)readerTool
{
    if (!_readerTool) {
        _readerTool = [[MyReaderTool alloc] init];
    }
    return _readerTool;
}
